"""
scripts/inspect_2018_raw.py
---------------------------
Deep inspection of 2018 raw data products for OceanEmbed.
...
def inspect_all_2018_products():
    raw_dir = Path("data/raw")

    print("=" * 80)
    print("OCEANEMBED 2018 RAW DATA PRODUCTS DEEP INSPECTION (365 DAYS)")
    print("=" * 80)

    # 1. SST
    sst_file = raw_dir / "sst" / "2018" / "sst_2018.nc"
    ...
    with xr.open_dataset(sst_file) as ds:
        ...
        assert len(ds.time) == 365, f"Expected 365 days for 2018 SST, got {len(ds.time)}"

    # 2. SSS
    sss_file = raw_dir / "sss" / "2018" / "sss_2018.nc"
    ...
    with xr.open_dataset(sss_file) as ds:
        ...
        assert len(ds.time) == 365, f"Expected 365 days for 2018 SSS, got {len(ds.time)}"

    # 3. SSH
    ssh_file = raw_dir / "ssh" / "2018" / "ssh_2018.nc"
    ...
    with xr.open_dataset(ssh_file) as ds:
        ...
        assert len(ds.time) == 365, f"Expected 365 days for 2018 SSH, got {len(ds.time)}"

    # 4. GLORYS
    glorys_dir = raw_dir / "glorys" / "2018"
    glorys_files = sorted(list(glorys_dir.glob("glorys_2018_m*.nc")))
    print(f"\n[4. GLORYS12V1 Reanalysis] Files: {len(glorys_files)} monthly files")
    assert len(glorys_files) == 12, f"Expected 12 monthly files for 2018 GLORYS, got {len(glorys_files)}"
    total_glorys_days = 0
    ...
    with xr.open_dataset(glorys_files[1]) as ds:
        print(f"  February 2018 days: {len(ds.time)} (standard year confirmed)")
        assert len(ds.time) == 28, f"Expected 28 days for Feb 2018, got {len(ds.time)}"

    # 5. OSCAR Currents
    curr_dir = raw_dir / "currents" / "2018"
    ...
    print("\n" + "=" * 80)
    print("ALL 6 RAW DATA PRODUCTS FOR YEAR 2018 VERIFIED COMPLETE AND UNCORRUPTED (365/365 DAYS)")
    print("=" * 80)

if __name__ == "__main__":
    inspect_all_2018_products()
