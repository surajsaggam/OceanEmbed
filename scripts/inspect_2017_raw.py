"""
scripts/inspect_2017_raw.py
---------------------------
Deep inspection of 2017 raw data products for OceanEmbed.
...
def inspect_all_2017_products():
    raw_dir = Path("data/raw")

    print("=" * 80)
    print("OCEANEMBED 2017 RAW DATA PRODUCTS DEEP INSPECTION (365 DAYS)")
    print("=" * 80)

    # 1. SST
    sst_file = raw_dir / "sst" / "2017" / "sst_2017.nc"
    ...
    with xr.open_dataset(sst_file) as ds:
        ...
        assert len(ds.time) == 365, f"Expected 365 days for 2017 SST, got {len(ds.time)}"

    # 2. SSS
    sss_file = raw_dir / "sss" / "2017" / "sss_2017.nc"
    ...
    with xr.open_dataset(sss_file) as ds:
        ...
        assert len(ds.time) == 365, f"Expected 365 days for 2017 SSS, got {len(ds.time)}"

    # 3. SSH
    ssh_file = raw_dir / "ssh" / "2017" / "ssh_2017.nc"
    ...
    with xr.open_dataset(ssh_file) as ds:
        ...
        assert len(ds.time) == 365, f"Expected 365 days for 2017 SSH, got {len(ds.time)}"

    # 4. GLORYS
    glorys_dir = raw_dir / "glorys" / "2017"
    glorys_files = sorted(list(glorys_dir.glob("glorys_2017_m*.nc")))
    print(f"\n[4. GLORYS12V1 Reanalysis] Files: {len(glorys_files)} monthly files")
    assert len(glorys_files) == 12, f"Expected 12 monthly files for 2017 GLORYS, got {len(glorys_files)}"
    total_glorys_days = 0
    for gf in glorys_files:
        with xr.open_dataset(gf) as ds:
            total_glorys_days += len(ds.time)
    print(f"  Total GLORYS days across 12 months: {total_glorys_days} (expected 365)")
    assert total_glorys_days == 365, f"Expected 365 total GLORYS days, got {total_glorys_days}"
    ...
    # Check Feb has 28 days
    with xr.open_dataset(glorys_files[1]) as ds:
        print(f"  February 2017 days: {len(ds.time)} (standard year confirmed)")
        assert len(ds.time) == 28, f"Expected 28 days for Feb 2017, got {len(ds.time)}"

    # 5. OSCAR Currents
    curr_dir = raw_dir / "currents" / "2017"
    curr_files = sorted(list(curr_dir.glob("*.nc")))
    print(f"\n[5. OSCAR Currents] Files: {len(curr_files)} daily files")
    assert len(curr_files) == 365, f"Expected 365 daily OSCAR files, got {len(curr_files)}"
    ...
    # 6. CCMP Winds
    winds_dir = raw_dir / "winds" / "2017"
    winds_files = sorted(list(winds_dir.glob("*.nc")))
    print(f"\n[6. CCMP Surface Winds] Files: {len(winds_files)} daily files")
    assert len(winds_files) == 365, f"Expected 365 daily CCMP files, got {len(winds_files)}"
    ...
    print("\n" + "=" * 80)
    print("ALL 6 RAW DATA PRODUCTS FOR YEAR 2017 VERIFIED COMPLETE AND UNCORRUPTED (365/365 DAYS)")
    print("=" * 80)

if __name__ == "__main__":
    inspect_all_2017_products()
