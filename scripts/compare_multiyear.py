"""
scripts/compare_multiyear.py
----------------------------
Scientific data-quality comparison across 2015, 2016, and 2017 interim datasets and raw storage.
...
def run_comparison():
    print("=" * 80)
    print("OCEANEMBED DATA-QUALITY COMPARISON: 2015 vs 2016 vs 2017")
    print("=" * 80)
    ...
    days_map = {2015: 365, 2016: 366, 2017: 365}
    ...
    if all(y in moments_dict for y in years):
        ...
        print(f"  {'Variable':10s} | {'2015 Mean':10s} | {'2016 Mean':10s} | {'2017 Mean':10s} | {'2015 Std':10s} | {'2016 Std':10s} | {'2017 Std':10s}")
        print("  " + "-" * 80)
        for var in ["SST", "SSS", "SSH", "U_curr", "V_curr", "WindU", "WindV"]:
            ...
            print(f"  {var:10s} | {means[2015]:10.4f} | {means[2016]:10.4f} | {means[2017]:10.4f} | {stds[2015]:10.4f} | {stds[2016]:10.4f} | {stds[2017]:10.4f}")

    # 3. Valid Counts per Physical Variable
    if all(y in moments_dict for y in years):
        print("\n[3. Valid Pixel Counts (Per Day Average)]")
        print(f"  {'Variable':10s} | {'2015 Pts/Day':14s} | {'2016 Pts/Day':14s} | {'2017 Pts/Day':14s}")
        print("  " + "-" * 60)
        for var in ["SST", "SSS", "SSH", "U_curr", "V_curr", "WindU", "WindV"]:
            c15 = moments_dict[2015][var]["count"] / 365.0
            c16 = moments_dict[2016][var]["count"] / 366.0
            c17 = moments_dict[2017][var]["count"] / 365.0
            print(f"  {var:10s} | {c15:14.1f} | {c16:14.1f} | {c17:14.1f}")
    ...
    if all(Path(f"data/interim/{y}").exists() for y in years):
        ...
        print(f"  {'Depth':6s} | {'2015 Pts/Day':14s} | {'2016 Pts/Day':14s} | {'2017 Pts/Day':14s} | {'Coverage %':12s}")
        print("  " + "-" * 70)
        for k, d in enumerate(depths_m):
            avg15 = t_counts[2015][k] / 365.0
            avg16 = t_counts[2016][k] / 366.0
            avg17 = t_counts[2017][k] / 365.0
            pct = (avg17 / (101 * 241)) * 100.0
            print(f"  {d:4d}m  | {avg15:14.1f} | {avg16:14.1f} | {avg17:14.1f} | {pct:11.2f}%")
